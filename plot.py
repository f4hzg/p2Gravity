...
def plot_acquisition(ob, ax = None, fiber_fov = 30):
...
def plot_dualObsExp(ob, template, ax = None, fiber_fov = 30, ft_ls = FT_LS, sc_ls = SC_LS, sobj = (0, 0), swap = 1):
    if ax is None:
        fig = plt.figure()
        ax = fig.add_subplot(111)
    # FT fiber
...
def plot_singleObsExp(ob, template, ax = None, fiber_fov = 30, ft_ls = FT_LS, sc_ls = SC_LS, swap = 1):
    if ax is None:
        fig = plt.figure()
        ax = fig.add_subplot(111)
    ax.plot(0, 0, "*k")
...
def plot_dualObsSwap(ob, template, ax = None, fiber_fov = 30, ft_ls = FT_LS, sc_ls = SC_LS, sobj = (0, 0), swap = 1):
    if ax is None:
        fig = plt.figure()
        ax = fig.add_subplot(111)
    if ob.ob_type in ["DualWideOnOb", "DualWideOffOb"]:
...
def plot_ob(ob, title = None, fov = None):
    ...
    plot_acquisition(ob, fiber_fov = fiber_fov, ax = ax_ob)
    if ob.ob_type in ["DualWideOnOb", "DualWideOffOb"]:
    ...
    for k in range(ntpl):
        ...
        if ob.ob_type in ["DualOffOb", "DualOnOb"]: # take into account acquisition offset
            ...
            plot_template(ob, ob.templates[k], fiber_fov = fiber_fov, ax = ax_ob, sobj = sobj, swap = swap)
        elif ob.ob_type in ["DualWideOnOb", "DualWideOffOb"]:
            plot_template(ob, ob.templates[k], fiber_fov = fiber_fov, ax = ax_ob, swap = swap)
        else:
            plot_template(ob, ob.templates[k], fiber_fov = fiber_fov, ax = ax_ob, swap = swap)
        ...
        if ob.ob_type in ["DualOffOb", "DualOnOb"]: # take into account acquisition offset
            sobj = (swap*ob.acquisition["SEQ.INS.SOBJ.X"], swap*ob.acquisition["SEQ.INS.SOBJ.Y"])
            txt_tpl = plot_template(ob, ob.templates[k], ax = ax_tpl, ft_ls = "-", sc_ls = "-", sobj = sobj)
        elif ob.ob_type in ["DualWideOnOb", "DualWideOffOb"]:
            txt_tpl = plot_template(ob, ob.templates[k], fiber_fov = fiber_fov, ax = ax_tpl, swap = swap)                        
        else:
            txt_tpl = plot_template(ob, ob.templates[k], ax = ax_tpl, ft_ls = "-", sc_ls = "-")            
        if k%2==0:
